...
# Enhanced prompt for batch processing with better structure
GEMINI_BATCH_PROMPT = """
You are analyzing images from NCERT (Indian educational) textbook chapters for a study assistant.
...
✓ **Maps**: Identify regions, features, legends
✓ **Math figures**: Explain constructions, theorems being illustrated
✓ **Historical photographs**: Describe people, events, settings
✓ **Experimental setups**: Detail apparatus and procedure
...
def initialize_multimodal_service():
    ...
    try:
            ...
            for model in genai.list_models():
                if 'generateContent' in model.supported_generation_methods:
                    ...
                    available_models_info.append({
                        'full': full_name,
                        'short': short_name,
                        'display_name': getattr(model, 'display_name', short_name)
                    })
            
            if available_models_info:
                print(f"✅ Found {len(available_models_info)} compatible models")
                
                # Priority order: 1.5-flash-latest > 1.5-flash > 1.5-pro > fallback
                flash_latest = [m for m in available_models_info if 'flash-latest' in m['short'].lower()]
                flash_models = [m for m in available_models_info if 'flash' in m['short'].lower() and 'latest' not in m['short'].lower()]
                pro_models = [m for m in available_models_info if 'pro' in m['short'].lower()]
                
                if flash_latest:
                    selected = flash_latest[0]
                elif flash_models:
                    selected = flash_models[0]
                elif pro_models:
                    selected = pro_models[0]
                else:
                    selected = available_models_info[0]
                
                model_name = selected['short']
        ...
        if not model_name:
            raise ValueError("Could not find any working Gemini model. Check your API key.")
        ...
        print(f"✅ Multimodal Service Initialized ({model_name} - BATCH MODE)")
...
def get_descriptions_for_batch(
    ...
    """
    Generates text descriptions for a BATCH of images using Gemini.
    Returns a list of analysis objects: [{"image_id": "...", "analysis": "..."}]
    """
    if not MODEL:
        raise Exception("Multimodal service (Gemini) not initialized.")
    if not image_bytes_batch or len(image_bytes_batch) > 16:
    ...
    try:
        ...
        # Build prompt
        full_prompt = GEMINI_BATCH_PROMPT.replace("{MANIFEST_PLACEHOLDER}", manifest_json)
        ...
        # Generate content
        print(f"📤 Sending {len(image_bytes_batch)} images to Gemini...")
        response = MODEL.generate_content(model_input)
        ...
        if hasattr(response, 'candidates') and response.candidates:
            ...
            if finish_reason == 'MAX_TOKENS':
                print(f"⚠️  Warning: Response truncated (hit token limit)")
        ...
        try:
            ...
            try:
                ...
                array_start = raw_text.find('"image_analyses"')
                if array_start != -1:
                    bracket_start = raw_text.find('[', array_start)
                    if bracket_start != -1:
                        ...
                        for i in range(bracket_start + 1, len(raw_text)):
                            char = raw_text[i]
                            ...
                            if not in_string:
                                if char == '{':
                                    ...
                                    if depth == 0:
                                        ...
                                        partial = raw_text[:i+1] + ']}'
                                        try:
                                            recovered = json.loads(partial)
                                            analyses = recovered.get("image_analyses", [])
                                            if analyses:
                                                print(f"⚠️  Recovered {len(analyses)} analyses from partial JSON")
                                                return analyses
                                        except:
                                            break
            except Exception as recovery_error:
